...
COPY . /go/src/github.com/xidongc/qproxy
WORKDIR /go/src/github.com/xidongc/qproxy
RUN make build/qproxy.linux

FROM alpine:3.7
RUN apk --no-cache add ca-certificates
COPY --from=builder /go/src/github.com/xidongc/qproxy/build/qproxy.linux /bin/qproxy
CMD ["/bin/qproxy"]
